...
```bash
python3 Eyes/scripts/price_scan_from_state.py --check-files
```
...
> | 标的类型 | 必须存在的文件 | 若缺失 |
> |:---|:---|:---|
> | 持仓 | `基础知识库.md` + `买入卖出备忘录.md` | 标注「建档不完整」，纳入雷达报告「待人工补充项」 |
> | Watchlist | `基础知识库.md` | 标注「未建档」，纳入调研建议 |
>
> **输出**：完整性检查结果写入 `Eyes/投资雷达/Raw_Data/YYYY-MM/files_check_YYYYMMDD.json`
