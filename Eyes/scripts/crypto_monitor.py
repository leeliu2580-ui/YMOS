...
def fetch_defillama_stablecoins():
    """Total stablecoin market cap from DeFiLlama stablecoins chart."""
    url = 'https://stablecoins.llama.fi/stablecoincharts/all'
    data = get(url)
    if '_error' in data or not isinstance(data, list) or len(data) == 0:
        return {}
    latest = data[-1]
    total = latest.get('totalCirculatingUSD', {}).get('peggedUSD', 0)
    return {'total_mcap': total, 'date': latest.get('date', '')}
...
def main():
    ...
    print('  [6/7] Hyperliquid Trades...')
    hl = fetch_hl_trades()

    print('  [7/8] Fear & Greed Index...')
    fg = fetch_fear_greed()

    print('  [8/8] DeFiLlama Stablecoins...')
    dllm = fetch_defillama_stablecoins()
...
| Metric | Value |
|:---|---:|
| **Total Stablecoin MCap** | **${dllm.get('total_mcap', 0)/1e9:.2f}B** |
| Date | {dllm.get('date', 'N/A')} |
...
- **Stablecoins:** ${usd_sc/1e9:.2f}B (CoinGecko) | ${dllm.get('total_mcap',0)/1e9:.2f}B (DeFiLlama)
