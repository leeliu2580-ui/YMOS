...
| 机会类型 | 特征 | YMOS 怎么帮你 |
|:---|:---|:---|
| **长线宏观 / 危机定价** | 宏观周期拐点、政策变化、黑天鹅事件创造的低估机会 | Eyes 模块持续监控宏观信号 + 事件异动，P13 自动识别战略级信号，Brain 通过 P2/P8/P9 评估定价是否合理 |
| **短线势能 / 中美映射** | 美股财报→A股映射、板块轮动、事件驱动的短期价差 | Eyes 实时追踪价格异动 + 关联传导，投资雷达自动匹配你的板块关联逻辑，策略分析给出具体路由建议 |
...
> V1 是 2024 年发布的早期版本（已归档至 [`v1-legacy` 分支](https://github.com/Evan-XYZ/YMOS/tree/v1-legacy)），只有 Eyes 模块的雏形。V3 是第一个完整的三模块版本。

| 维度 | V1 | V3 | 为什么重要 |
|:---|:---|:---|:---|
| **数据源** | 仅 RSS 抓取 | Finnhub → Tushare → Yahoo **三源价格路由** + RSS + 结构化 API | 美股/A股/Crypto 实时报价自动分流，不再手动查价格 |
| **市场分析** | 新闻摘要（简易版） | P13 标准模板：市场体温 + 战略信号仪表盘 + 势能分析 + 风险警告 | 从"看新闻"升级为"识别信号"，结构化输出可追溯 |
...
**一句话总结**：V1 只帮你"看新闻"，V3 帮你"看新闻 → 识别信号 → 分析策略 → 纪律审查 → 你决策"的完整闭环。
...
把旧的 YMOS 文件夹重命名为 `YMOS-V1`（保留备份），然后下载 V3 到新的 `YMOS/` 文件夹，跑一遍入职引导（`开始使用`）即可。V3 完全取代 V1，不需要迁移数据——你在 V1 里的研报和笔记可以手动拖入 V3 的个股文件夹。
...
是的。所有产出物都是 Markdown 文件，存在你自己的 Obsidian Vault 里。没有云端数据库、没有 SaaS 依赖、没有数据上传。你的投资数据完全由你掌控。
