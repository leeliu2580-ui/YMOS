...
| 文件 | 性质 | 作用层级 |
|:---|:---|:---|
| **`AGENTS.md`** | OpenClaw 通用工作区模板 | 基础层：Session 启动流程、记忆系统规范、群聊礼仪、心跳机制、安全红线 |
| **`AGENT_GUIDE.md`**（本文件） | YMOS 垂直投研 SOP 操作手册 | 应用层：Eyes/Brain/持仓与关注 模块路由、文件权限、状态机写回规则 |

**层叠原则**：
- `AGENTS.md` 覆盖 YMOS 未定义的通用行为（记忆、群聊、心跳、安全等）
- YMOS 垂直规则（`AGENT_GUIDE.md` / `总入口暗号.md` / SOP 文件）覆盖 `AGENTS.md` 的对应条目
- 两者协同生效，不是二选一

**Session 启动时**，按以下顺序读取：
1. `SOUL.md` — 通用人格风格（先知道自己是谁）
2. `USER.md` — 用户画像（再知道在帮谁）
3. `总入口暗号.md` — YMOS 工作流路由（然后决定做什么）
4. `AGENT_GUIDE.md`（本文件）— 拿到具体 SOP 操作手册
