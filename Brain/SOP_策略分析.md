# 🎯 策略分析 SOP

> 暗号：`我想买 [ticker]` / `我想卖 [ticker]` / `做个仓位再平衡` / `跑一下策略分析`
> 模块：Brain/（大脑 — 策略路由 + 分析）

---

## 一句话定位

策略分析专注回答一件事：**基于分析结论和当前仓位，现在应该做什么动作，怎么做**

- 策略分析是下游模块，依赖 Eyes（投资雷达）的输出作为触发
- 它本身不做信息收集，也不做市场洞察
- 核心原则：**能跑的先跑完，不阻塞**

---

## 🔑 触发暗号

| 暗号 | 动作意图 | 跳转路由 |
|:---|:---|:---|
| `我想买 [ticker]` | 首次建仓 | → 路由 A：买入 |
| `加仓 [ticker]` | 已有仓位再加 | → 路由 B：加仓 |
| `我想卖 [ticker]` | 减仓/清仓 | → 路由 D：卖出 |
| `持有怎么看 [ticker]` | 持有评估 | → 路由 C：持有 |
| `做个仓位再平衡` | 组合级调整 | → 路由 E：再平衡 |
| `跑一下策略分析` | 定时/手动批量 | → 自动模式（读雷达 → 分流） |

---

## ⚙️ 完整执行步骤

### Step 0：资产类别识别与路由分流

在执行任何策略前，AI 必须识别标的的资产类别：

1. **读取状态机**：检查 `持仓_状态机.md` 或 `Watchlist_状态机.md` 的「市场」列。
2. **分流逻辑**：
   - **若为 Crypto**：
     - 若意图为 `持有怎么看` -> **跳转至：[Brain/SOP_仓位复盘.md](Brain/SOP_仓位复盘.md)**。
     - 若意图为 `调研一下` -> **跳转至：加密调研流程（使用 crypto-snapshot Skill）**。
     - 其他意图（买/卖） -> 继续执行本 SOP，但数据获取应优先尝试加密相关脚本。
   - **若为美股/港股/A股/其他**：
     - 继续执行本 SOP。

---

### Step 1：确定触发来源与目标

**1.1 手动暗号触发**（`我想买/卖/加仓/持有怎么看`）：
- 目标标的 = 用户指定的 ticker
- 动作意图 = 暗号直接指定
- 跳到 Step 3

**1.2 定时任务/自动触发**（`跑一下策略分析`）：
- 读取最新投资雷达报告：`Eyes/投资雷达/YYYY-MM/投资雷达_YYYY-MM-DD.md`
  - 若今日无雷达 → 查找最近一份
- 提取雷达报告中 `## 🔭 下一步建议 > 策略分析建议` 区块
- 从中筛选需要策略分析的标的和动作意图

**1.3 消费调研建议（1.2 同时执行）**：
- 同样从投资雷达报告中提取 `## 🔭 下一步建议 > 调研建议` 区块
- 对每个「建议跑 调研一下 TICKER」的标的：
  - 调用 `Brain/SOP_初始调研.md` 执行 P1+P4+P2 补全
  - 完成后记录到策略分析日志的「本次自动调研」section
- 若无调研建议 → 跳过
- **先完成 1.3 调研，再进入 Step 2**（确保后续策略路由有完整的个股上下文）

> 为什么在策略分析里消费调研建议？调研完成后 P1/P4 补全，后续策略路由可直接使用完整上下文，不需要用户手动 `调研一下`。

**1.2 + 1.3 均无建议时** → 输出「雷达无策略分析建议，本次跳过」→ 退出

### Step 2：加载用户上下文

按顺序读取：

```
持仓与关注/当前关注方向与投资偏好.md          ← 必须存在，否则阻塞
持仓与关注/持仓_状态机.md
持仓与关注/Watchlist_状态机.md
```

> ⛔ 如果 `当前关注方向与投资偏好.md` 不存在或为空 → 停止，输出：
> 「前置条件不足：缺少当前关注方向与投资偏好。请先填写（路径：持仓与关注/当前关注方向与投资偏好.md）」

### Step 3：逐标的前置检查与补足

对每个目标标的执行以下检查：

| # | 检查项 | 文件路径 | 若缺失 |
|:---|:---|:---|:---|
| ① | 个股基础知识库 | `持仓与关注/{位置}/名称_TICKER/个股基础知识库.md` | **调用 Brain/SOP_初始调研.md 补足** → 继续 |
| ② | P1 基石档案 | 知识库内 `## P1 基石档案` 区块 | **调用初始调研补足** → 继续 |
| ③ | P4 重点关注点 | 知识库内 `## P4 重点关注点` 区块 | **调用初始调研补足** → 继续 |
| ④ | P2 阶段判断 | 知识库内 `## P2 阶段判断` 区块 | **AI 自动执行 P2** → 继续 |
| ⑤ | 买入卖出备忘录 | `持仓与关注/{位置}/名称_TICKER/买入卖出备忘录.md` | **不阻塞**，分析完后提示补充 |
| ⑥ | 当前关注方向与投资偏好 | `持仓与关注/当前关注方向与投资偏好.md` | ⛔ 阻塞（Step 2 已检查） |

**不阻塞原则**：
- ①②③ 缺失 → 调用初始调研子模块补足后继续
- ④ 缺失 → AI 自动执行 P2
- ⑤ 缺失 → 继续执行，报告末尾标注「优化建议」
- 只有 ⑥ 缺失才阻塞

**个股上下文范围**：读取该标的文件夹下的**所有文件**作为分析上下文（知识库 + 备忘录 + 用户手动拖入的文件 + 历史策略报告）

### Step 4：P4 新鲜度纪律检查

**每次调仓动作后必须执行**

```bash
python3 Eyes/scripts/price_scan_from_state.py --check-p4
```

> **动态读取标的**：脚本自动从 `持仓_状态机.md` + `Watchlist_状态机.md` 读取 Active 标的列表，不硬编码。
>
> **检查内容**：逐标的检查 `基础知识库.md` 是否包含 `## P4 重点关注点` 区块，以及该区块的更新时间。
>
> **新鲜度规则**：
> | P4 状态 | 判定 | 动作 |
> |:---|:---|:---|
> | 超过 30 天未更新 | ⚠️ 信息过期 | 纳入调研建议，提示 `建议跑 调研一下 TICKER` |
> | P4 存在但无日期戳 | ⚠️ 格式不完整 | 标注「待补日期」 |
> | 无 P4 区块 | ❌ 未建档 | 纳入调研建议 |
> | 30 天内已更新 | ✅ 合规 | 无需操作 |
>
> **输出**：新鲜度检查结果写入 `Brain/策略分析/Raw_Data/YYYY-MM/p4_check_YYYYMMDD.json`

> **为什么在策略分析里检查 P4**：
> - 调仓动作（买/卖/加仓）是「主动出击」，最容易忽略档案维护
> - 此时检查 P4 新鲜度，可以确保分析结论有最新的关注点支撑
> - 配合雷达 Step 4 的档案完整性预检，形成「投前 + 投后」双重档案纪律

### Step 5：执行目录初始化

```bash
mkdir -p "Brain/策略分析/Raw_Data/$(date +%Y-%m)" \
         "Brain/策略分析/$(date +%Y-%m)"
```

### Step 6：执行策略路由

根据动作意图分流到对应路由：

---

#### 路由 A：买入（首次建仓）

**提示词链**：
```
P2（复用 Step 3 结论）
  ↓
P9 反向DCF 估值 → Brain/references/p9-valuation.md
  ↓
P5 FOMO Killer → Brain/references/p5-fomo-killer.md
  ↓
[可选] P10 期权策略 → Brain/references/p10-options.md
  ↓
P12 纪律审查 → Brain/references/p12-referee.md
  ↓
Human 最终决策 ✋
```

**写回**：
1. `Watchlist_状态机.md` → 更新为「已建仓」
2. 对应标的 `买入卖出备忘录.md` → 追加买入记录
3. `持仓_状态机.md` → 新增标的
4. 将标的从 `动态Watchlist/` 迁移至 `持仓/`
5. 归档到 `Brain/策略分析/YYYY-MM/YYYY-MM-DD_TICKER_买入.md`
6. → **最终挪入个股文件夹**

---

#### 路由 B：加仓

**提示词链**：
```
P2 → P3/P9（事件/价格触发） → P5 FOMO Killer → P12 纪律审查 → Human ✋
```

**写回**：
1. `买入卖出备忘录.md` → 追加加仓记录
2. `持仓_状态机.md` → 更新成本价
3. 归档 → 挪入个股文件夹

---

#### 路由 C：持有评估

**提示词链**：
```
P2 → P6 利润守门员 → [事件]P3 → [宏观]P8 → P12 纪律审查 → Human ✋
```

**写回**：
1. `买入卖出备忘录.md` → 追加评估记录
2. `持仓_状态机.md` → 更新监控价位
3. 归档 → 挪入个股文件夹

---

#### 路由 D：减仓 / 卖出

**提示词链**：
```
读取备忘录（原始买入理由） → P2 → P6 → [可选]P10 → P12 → Human ✋
  ↓
[强制] P11 交易复盘 → Brain/references/p11-autopsy.md
```

> ⚠️ **卖出前三问**：
> 1. 买入核心逻辑是否已被证伪？
> 2. 卖出是基于策略还是恐慌？
> 3. 再出现利好会后悔吗？

**写回**：
1. `买入卖出备忘录.md` → 追加卖出记录
2. `持仓_状态机.md` → 移除或降级
3. 完全清仓：标的从 `持仓/` 移入 `动态Watchlist/`
4. 归档 → 挪入个股文件夹
5. **强制复盘**：在 `持仓与关注/投资复盘/` 生成复盘报告，并将核心教训追加至 `memory/experience_base.md`

---

#### 路由 E：仓位再平衡

**提示词链**：
```
持仓_状态机 → 当前关注方向与投资偏好 → P7 组合再平衡 → [逐个]P6 → P12 → Human ✋
```

**写回**：
1. `持仓_状态机.md` → 更新权重
2. 各标的 `买入卖出备忘录.md` → 追加记录
3. 归档到 `Brain/策略分析/YYYY-MM/YYYY-MM-DD_仓位再平衡.md`

---

### Step 7：写回产出物

每次策略分析**必须完成以下写回**：

**A. 策略报告**
- `Brain/策略分析/YYYY-MM/YYYY-MM-DD_TICKER_动作类型.md`（成品报告）
- `Brain/策略分析/Raw_Data/YYYY-MM/strategy_context_YYYYMMDD_TICKER_动作.json`（中间件）
- **个股报告最终挪入对应标的文件夹**（`持仓与关注/{位置}/名称_TICKER/`）

**B. 状态更新**
- `持仓_状态机.md` / `Watchlist_状态机.md`
- 对应标的 `买入卖出备忘录.md`

**B+. 个股知识库增量更新**
- 若本次分析执行了 P2 → 将结论增量写回 `持仓与关注/{位置}/名称_TICKER/个股基础知识库.md` 的 `## P2 阶段判断` 区块
  - 已有旧 P2 结论 → **替换**为最新结论（保留日期戳，如 `> 更新于 YYYY-MM-DD`）
  - 无此区块 → **新建追加**到知识库末尾
- 目的：下次策略分析 Step 3 前置检查时，P2 不再缺失，避免重复执行

**C. 策略分析日志（当日汇总）** ⭐
- 路径：`Brain/策略分析/YYYY-MM/策略分析日志_YYYY-MM-DD.md`
- 每次策略分析完成后，追加条目到当日日志
- 日志格式：

```markdown
# 策略分析日志 - YYYY-MM-DD

## 今日分析记录

| 时间 | 标的 | 动作 | 路由 | 结论摘要 | 报告路径 | 状态 |
|:---|:---|:---|:---|:---|:---|:---|
| HH:MM | TICKER | 持有评估 | C | P2:PVE P6:持有 P12:通过 | 持仓与关注/持仓/XX/... | 待确认 |

## 📋 本次自动调研（Step 1.3）

| 标的 | 触发原因 | 完成状态 |
|:---|:---|:---|
| TICKER | P1 缺失 / 信息过期 / 新关注未建档 | ✅ 完成 / ❌ 失败（原因） |

（无调研建议时此 section 可省略）

## 待用户确认

- [ ] TICKER：P12 通过，建议执行 `建议跑 我想买 TICKER`
- [ ] TICKER：止盈触发，建议确认是否执行

## 宏观分析（如有）

（当日宏观事件分析结论，不针对单标的）

## 下一步建议

- 建议优先处理：XXX
- 信息缺口：XXX 缺少最新财报数据，建议补充
```

**D. 缺失项提示**

仅当存在缺失项时，在策略报告末尾追加：
```markdown
## 📌 优化建议
- 建议补充买入卖出备忘录：`持仓与关注/{位置}/名称_TICKER/买入卖出备忘录.md`
```

> 注意：P2 写回已在 B+ 步骤自动完成，不再列入优化建议。

### Step 8：在对话中输出策略报告

直接输出完整策略分析报告内容。

---

## 📦 产出物清单

| 文件 | 路径 | 说明 |
|:---|:---|:---|
| 策略分析报告 | `Brain/策略分析/YYYY-MM/` → 最终挪入个股文件夹 | 按日-标的归档 |
| 策略上下文（Raw） | `Brain/策略分析/Raw_Data/YYYY-MM/` | 中间件 |
| 策略分析日志 | `Brain/策略分析/YYYY-MM/` | 当日汇总 |
| 更新：状态机 | `持仓与关注/` | 状态 + P4 |
| 更新：单标的 | `持仓与关注/{位置}/名称_TICKER/` | 备忘录 + 策略报告 |

---

## 📁 路径速查

| 内容 | 路径 |
|:---|:---|
| 当前关注方向与投资偏好 | `持仓与关注/当前关注方向与投资偏好.md` |
| 持仓状态机 | `持仓与关注/持仓_状态机.md` |
| Watchlist 状态机 | `持仓与关注/Watchlist_状态机.md` |
| 个股知识库 | `持仓与关注/{位置}/名称_TICKER/个股基础知识库.md` |
| 买入卖出备忘录 | `持仓与关注/{位置}/名称_TICKER/买入卖出备忘录.md` |
| 最新投资雷达 | `Eyes/投资雷达/YYYY-MM/投资雷达_YYYY-MM-DD.md` |
| 策略归档 | `Brain/策略分析/YYYY-MM/` |
| 策略分析日志 | `Brain/策略分析/YYYY-MM/策略分析日志_YYYY-MM-DD.md` |
| 初始调研 SOP | `Brain/SOP_初始调研.md` |
| P 提示词目录 | `Brain/references/` |

---

## 🔮 P 提示词模块（策略层）

| 提示词 | 场景 | 路径 |
|:---|:---|:---|
| P2 Phase Check | 阶段判断 — **每次必跑** | `Brain/references/p2-phase-check.md` |
| P3 Event Impact | 事件冲击 | `Brain/references/p3-event-impact.md` |
| P5 FOMO Killer | 买入/加仓审计 | `Brain/references/p5-fomo-killer.md` |
| P6 Profit Keeper | 持有/卖出审计 | `Brain/references/p6-profit-keeper.md` |
| P7 Portfolio Check | 组合再平衡 | `Brain/references/p7-portfolio-check.md` |
| P8 Macro Filter | 宏观过滤 | `Brain/references/p8-macro-filter.md` |
| P9 Valuation | 反向DCF估值 | `Brain/references/p9-valuation.md` |
| P10 Options | 期权策略 | `Brain/references/p10-options.md` |
| P11 Autopsy | 交易复盘 | `Brain/references/p11-autopsy.md` |
| P12 Referee | 纪律审查 — **每次最终必过** | `Brain/references/p12-referee.md` |

---

## ⚠️ 边界与反模式

**策略分析不做**：
- 不做市场洞察（Eyes 的事）
- 不做信号发现（投资雷达的事）
- 不自动执行买卖（Human in the Loop）

**反模式**：
- 跳过 P2 直接进 P5/P6
- 没有 P12 就给出买卖建议
- 没有读取备忘录就做卖出
- 策略分析没有归档
- 因前置缺失就完全不跑（能跑的先跑）

---

*SOP 版本：2026-03-18 · YMOS V3 三模块制（Eyes / Brain / 持仓与关注）*
